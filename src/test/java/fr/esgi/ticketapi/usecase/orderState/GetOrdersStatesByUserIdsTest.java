...
@SpringBootTest
public class GetOrdersStatesByUserIdsTest {
    GetOrdersStatsByUserIds getOrdersStatesByUserIds;

    GetOrdersStatsByUserIds getOrdersStatesByUserIdsWithMockedRepo;
    ...
    public void setup() {
        ...
        getOrdersStatesByUserIds = new GetOrdersStatsByUserIds(mockOrderStateDao, mockOrderDao);
        getOrdersStatesByUserIdsWithMockedRepo = new GetOrdersStatsByUserIds(realOrderStateDao, mockOrderDao);
    ...
    public void should_call_orderDao_to_get_user_orders() {
        userIds.add(123);
        getOrdersStatesByUserIds.execute(userIds);
    ...
    public void should_call_orderStateDao_to_get_user_orders_states() {
        ...
        Mockito.verify(mockOrderStateDao, Mockito.times(userIds.size())).getStatesOfOrderIds(mockOrders.stream().map(Order::getId).collect(Collectors.toList()));
    }

    @Test
    public void should_return_last_order_state_of_each_user_orders() {
        userIds.add(123);
        List<UserOrderState> result = getOrdersStatesByUserIdsWithMockedRepo.execute(userIds);
        List<UserOrderState> expected = new ArrayList<>();
        List<OrderState> userOrdersExpected = new ArrayList<>();
        userOrdersExpected.add(new OrderState(1,1,1, LocalDate.now().minusDays(2)));
